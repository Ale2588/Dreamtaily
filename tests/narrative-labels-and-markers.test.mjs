...
test("frontend renders the author label directly and never invents numbered options",()=>{
  assert.match(frontend,/dtOptionLabel\(option\)/);
...
test("branch cards use authored labels and descriptions without legacy hardcodes",()=>{
  ...
  assert.match(frontend,/function dtOptionLabel\(option\)/);
  assert.match(frontend,/prima opzione\|seconda opzione/);
  assert.match(frontend,/destination\?\.title/);
